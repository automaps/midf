...
from midf.imdf_model import (
  IMDFAddress, IMDFAmenity, IMDFAnchor, IMDFDetail, IMDFFeature,
  IMDFFixture, IMDFFootprint, IMDFGeofence, IMDFKiosk, IMDFLevel, IMDFOpening, IMDFSection, IMDFUnit,
  IMDFVenue,
  )
from .exceptions import (
  FeatureGeometryTypeInvalidError, GeometryMustBeValidError,
  )
  ...
  if hasattr(feature, "geometry"):
    if feature.geometry:
      ...
      if not feature.geometry.is_valid:
        raise GeometryMustBeValidError(f"Invalid geometry for {feature.id}", feature.id)
