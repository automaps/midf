...
from midf.imdf_model import (
  IMDFAddress, IMDFAmenity, IMDFAnchor, IMDFBuilding, IMDFDetail, IMDFFeature, IMDFFixture, IMDFFootprint,
  IMDFGeofence, IMDFKiosk, IMDFLevel, IMDFManifest, IMDFOccupant, IMDFOpening, IMDFRelationship, IMDFSection,
  IMDFUnit, IMDFVenue,
  )
from .base import BaseValidator, ValidationError, validate_geometry
from .exceptions import (
  AnchorMustHaveGeoreferenceError, BuildingMustHaveCategoryError, BuildingMustHaveNameError,
  DetailMustHaveCategoryError, FixtureMustHaveCategoryError, FootprintMustHaveBuildingIdsError,
  GeofenceMustHaveCategoryError, IMDFValidationError, KioskMustHaveCategoryError, LevelMustHaveOrdinalError,
  LevelMustHaveShortNameError, ManifestCreatedDateMustBeValidError, ManifestMustHaveCreatedDateError,
  ManifestMustHaveGeneratedByError, ManifestMustHaveLanguageError, ManifestMustHaveVersionError,
  ManifestVersionMustBeValidError, OccupantMustHaveCategoryError, OccupantMustHaveNameError,
  OpeningMustHaveCategoryError, ReferencedFeatureIDMustBeResolvableError, RelationshipMustHaveCategoryError,
  RelationshipMustHaveDirectionError, SectionMustHaveNameError, UnitMustHaveCategoryError,
  UnitMustHaveLevelIdError, VenueMustHaveAddressIdError, VenueMustHaveCategoryError, VenueMustHaveNameError,
  )
...
class BuildingValidator(BaseValidator):
  ...
  def validate(self, feature: IMDFFeature) -> None:
    ...
    if not feature.name:
      raise BuildingMustHaveNameError(f"Building {feature.id} is missing required field 'name'", feature.id)
    if not feature.category:
      raise BuildingMustHaveCategoryError(
          f"Building {feature.id} is missing required field 'category'", feature.id
          )
...
class LevelValidator(BaseValidator):
  ...
  def validate(self, feature: IMDFFeature) -> None:
    ...
    if feature.ordinal is None:
      raise LevelMustHaveOrdinalError(f"Level {feature.id} is missing required field 'ordinal'", feature.id)
...
class UnitValidator(BaseValidator):
  ...
  def validate(self, feature: IMDFFeature) -> None:
    ...
    if not feature.category:
      raise UnitMustHaveCategoryError(f"Unit {feature.id} is missing required field 'category'", feature.id)
    if not feature.level_id:
      raise UnitMustHaveLevelIdError(f"Unit {feature.id} is missing required field 'level_id'", feature.id)
...
class VenueValidator(BaseValidator):
  ...
  def validate(self, feature: IMDFFeature) -> None:
    ...
    if not feature.name:
      raise VenueMustHaveNameError(f"Venue {feature.id} is missing required field 'name'", feature.id)
    if not feature.category:
      raise VenueMustHaveCategoryError(f"Venue {feature.id} is missing required field 'category'", feature.id)
