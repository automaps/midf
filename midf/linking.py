...
from midf.imdf_model import (
  IMDFAddress, IMDFAnchor, IMDFBuilding, IMDFDetail, IMDFFeature, IMDFFeatureType, IMDFFixture, IMDFFootprint,
  IMDFGeofence, IMDFKiosk,
  IMDFLevel,
  IMDFManifest,
  IMDFOccupant, IMDFOpening, IMDFRelationship, IMDFSection, IMDFUnit, IMDFVenue,
  )
from midf.loading import MANIFEST_KEY
from midf.model import (
  MIDFAddress, MIDFAnchor, MIDFBuilding, MIDFDetail, MIDFFixture, MIDFFootprint, MIDFGeofence, MIDFKiosk,
  MIDFLevel,
  MIDFManifest,
  MIDFOccupant, MIDFOpening,
  MIDFRelationship, MIDFSection,
  MIDFSolution, MIDFUnit, MIDFVenue,
  )
...
def link_imdf(
    imdf_dict: Mapping[Union[str, IMDFFeatureType], Collection[Union[IMDFManifest, IMDFFeature]]]
    ) -> MIDFSolution:
  ...
  for venue in imdf_dict[IMDFFeatureType.venue]:
    ...
    venue_mapping[venue.address_id].append(
        MIDFVenue(
            id=venue.id, geometry=venue.geometry, name=venue.name, category=venue.category,
            display_point=venue.display_point, restriction=venue.restriction, alt_name=venue.alt_name,
            hours=venue.hours, phone=venue.phone, website=venue.website
            )
        )
  ...
  for address in imdf_dict[IMDFFeatureType.address]:
    ...
    addresses[address.id] = MIDFAddress(
        ...
        venues=venue_mapping.pop(address.id) if address.id in found_venue_addresses else None,
        )
  ...
  for footprint in imdf_dict[IMDFFeatureType.footprint]:
    ...
    footprints[footprint.id] = MIDFFootprint(
        ...
        buildings=[buildings[b] for b in footprint.building_ids] if footprint.building_ids else None,
        )
  ...
  for anchor in imdf_dict[IMDFFeatureType.anchor]:
    ...
    anchors[anchor.unit_id].append(
        MIDFAnchor(
            ...
            # address=addresses[section.address_id], # TODO: INVALID IMDF!
            occupants=occupants.pop(anchor.id) if anchor.id in found_occupant_anchors else None,
            )
        )
  ...
  for unit in imdf_dict[IMDFFeatureType.unit]:
    ...
    units[unit.level_id].append(
        MIDFUnit(
            ...
            anchors=anchors.pop(unit.id) if unit.id in found_anchor_unit_ids else None,
            )
        )
  ...
  for fixture in imdf_dict[IMDFFeatureType.fixture]:
    ...
    fixtures[fixture.level_id].append(
        MIDFFixture(
            ...
            anchor=anchor_id_mapping[fixture.anchor_id] if fixture.anchor_id else None,
            )
        )
  ...
  for level in imdf_dict[IMDFFeatureType.level]:
    ...
    levels[level.id] = MIDFLevel(
        ...
        buildings=[buildings[b] for b in level.building_ids] if level.building_ids else None,
        sections=sections.pop(level.id) if level.id in found_section_levels else None,
        kiosks=kiosks.pop(level.id) if level.id in found_kiosk_levels else None,
        fixtures=fixtures.pop(level.id) if level.id in found_fixture_levels else None,
        openings=openings.pop(level.id) if level.id in found_opening_levels else None,
