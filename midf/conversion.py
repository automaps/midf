...
def to_mi_solution(midf_solution: MIDFSolution) -> Solution:
  ...
  for occupant_category in IMDFOccupantCategory:
    occupant_category_mapping[occupant_category] = mi_solution.add_occupant_category(occupant_category.name)
        ...
        if unit.anchors:
          for anchor in unit.anchors:
            anchor_key = mi_solution.add_point_of_interest(
                admin_id=anchor.id, name=anchor_name, point=anchor.geometry, floor_key=floor_key,
            ...
            for occupant in anchor.occupants:
              ...
              occupant_template_key = mi_solution.add_occupant_template(
                  ...
                  occupant_category_key=occupant_category_mapping[occupant.category],
                  # business_hours=occupant.hours,
                  # contact=occupant.phone + occupant.website,

                  )
              mi_solution.add_occupant(location_key=anchor_key, occupant_template_key=occupant_template_key)
